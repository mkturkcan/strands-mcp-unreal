...
void UMassProcessor_CrowdVisualizationCustomData::UpdateCrowdCustomData(FMassExecutionContext& Context)
{
	...
	for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
	{
		...
		const FMassViewerInfoFragment& LODInfo = LODInfoList[EntityIt];
		const FMassRepresentationLODFragment& RepresentationLOD = RepresentationLODList[EntityIt];
		const FCitySampleCrowdVisualizationFragment& CitySampleCrowdVisualization = CitySampleCrowdVisualizationList[EntityIt];
			 ...
			 (Representation.CurrentRepresentation == EMassRepresentationType::LowResSpawnedActor || Representation.CurrentRepresentation == EMassRepresentationType::HighResSpawnedActor)) )
		{
			...
			//if (ExclusiveInstanceViewerIdx == INDEX_NONE || LODInfo.bIsVisibleByViewer[ExclusiveInstanceViewerIdx])
			{
				...
				// Update Transform
				UMassUpdateISMProcessor::UpdateISMTransform(Context.GetEntity(EntityIt), ISMInfo, TransformFragment.GetTransform()
