...
{
	const bool bIsGauntletRun = FParse::Param(FCommandLine::Get(), TEXT("gauntlet"));
	// Do not attempt to load a save game in an automated run or when running unattended. This may be not good for the consistency, but more importantly,
	// the save file may become corrupted and the attempt to load it will pop up a modal dialog
	if (bIsGauntletRun || FApp::IsUnattended())
	{
		SaveGame = nullptr;
	}
	else
