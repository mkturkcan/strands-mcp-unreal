...
UCitySamplePanel* UCitySamplePanel::AddChildPanel(UCitySamplePanel* ChildPanel, UPanelWidget* ContainerWidget, bool bSkipAnimation)
{
	...
	if (FWidgetAnimationHandle Handle = ChildPanel->AnimateIn())
	{
		// Map child to be added to the animation sequence player
		AnimateInPanelMapping.Add(Handle, ChildPanel);
		// Bind the final steps for adding the child panel to the animation finished event
		Handle.GetAnimationState()->GetOnWidgetAnimationFinished().AddUObject(this, &UCitySamplePanel::OnAnimateIn);

		if (bSkipAnimation)
		{
			Handle.GetAnimationState()->Stop();
		}
	}
	else
	{
...
void UCitySamplePanel::RemoveChildPanelChecked_Internal(UCitySamplePanel* ChildPanel, bool bSkipAnimation /*= false*/)
{
	...
	if (FWidgetAnimationHandle const* OutroHandle = AnimateOutPanelMapping.FindKey(ChildPanel))
	{
		if (bSkipAnimation)
		{
			...
			(*OutroHandle).GetAnimationState()->Stop();
		}
	...
	if (FWidgetAnimationHandle const* IntroHandle = AnimateInPanelMapping.FindKey(ChildPanel))
	{
		(*IntroHandle).GetAnimationState()->Stop();
	}
	...
	if (FWidgetAnimationHandle Handle = ChildPanel->AnimateOut())
	{
		// Map child to be removed to the animation sequence player
		AnimateOutPanelMapping.Add(Handle, ChildPanel);
		// Bind the final steps for removing the child panel to the animation finished event
		Handle.GetAnimationState()->GetOnWidgetAnimationFinished().AddUObject(this, &UCitySamplePanel::OnAnimateOut);
...
void UCitySamplePanel::OnAnimateIn(FWidgetAnimationState& AnimationState)
{
	FWidgetAnimationHandle Handle = AnimationState.GetAnimationHandle();
	check(AnimateInPanelMapping.Contains(Handle));

	UCitySamplePanel* ChildPanel = AnimateInPanelMapping[Handle];
	AnimateInPanelMapping.Remove(Handle);
	AnimationState.GetOnWidgetAnimationFinished().RemoveAll(this);

	AddChildPanelHelper(ChildPanel);
}

void UCitySamplePanel::OnAnimateOut(FWidgetAnimationState& AnimationState)
{
	FWidgetAnimationHandle Handle = AnimationState.GetAnimationHandle();
	check(AnimateOutPanelMapping.Contains(Handle));

	UCitySamplePanel* ChildPanel = AnimateOutPanelMapping[Handle];
	AnimateOutPanelMapping.Remove(Handle);
	AnimationState.GetOnWidgetAnimationFinished().RemoveAll(this);
